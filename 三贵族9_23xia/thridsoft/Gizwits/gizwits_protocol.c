...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagfan_button)
    {
        info->event[info->num] = EVENT_fan_button;
        info->num++;
        dataPoints->valuefan_button = gizStandardDecompressionValue(fan_button_BYTEOFFSET,fan_button_BITOFFSET,fan_button_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagwater_button)
    {
        info->event[info->num] = EVENT_water_button;
        info->num++;
        dataPoints->valuewater_button = gizStandardDecompressionValue(water_button_BYTEOFFSET,water_button_BITOFFSET,water_button_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagled_button)
    {
        info->event[info->num] = EVENT_led_button;
        info->num++;
        dataPoints->valueled_button = gizStandardDecompressionValue(led_button_BYTEOFFSET,led_button_BITOFFSET,led_button_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagmotov_button)
    {
        info->event[info->num] = EVENT_motov_button;
        info->num++;
        dataPoints->valuemotov_button = gizStandardDecompressionValue(motov_button_BYTEOFFSET,motov_button_BITOFFSET,motov_button_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagbeep_button)
    {
        info->event[info->num] = EVENT_beep_button;
        info->num++;
        dataPoints->valuebeep_button = gizStandardDecompressionValue(beep_button_BYTEOFFSET,beep_button_BITOFFSET,beep_button_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuefan_button != cur->valuefan_button)
    {
        GIZWITS_LOG("valuefan_button Changed\n");
        ret = 1;
    }
    if(last->valuewater_button != cur->valuewater_button)
    {
        GIZWITS_LOG("valuewater_button Changed\n");
        ret = 1;
    }
    if(last->valueled_button != cur->valueled_button)
    {
        GIZWITS_LOG("valueled_button Changed\n");
        ret = 1;
    }
    if(last->valuemotov_button != cur->valuemotov_button)
    {
        GIZWITS_LOG("valuemotov_button Changed\n");
        ret = 1;
    }
    if(last->valuebeep_button != cur->valuebeep_button)
    {
        GIZWITS_LOG("valuebeep_button Changed\n");
        ret = 1;
    }
    ...
    if(last->valuesmoke != cur->valuesmoke)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuesmoke Changed\n");
            ret = 1;
        }
    }
    if(last->valueethanol != cur->valueethanol)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueethanol Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(fan_button_BYTEOFFSET,fan_button_BITOFFSET,fan_button_LEN,(uint8_t *)devStatusPtr,dataPoints->valuefan_button);
    gizStandardCompressValue(water_button_BYTEOFFSET,water_button_BITOFFSET,water_button_LEN,(uint8_t *)devStatusPtr,dataPoints->valuewater_button);
    gizStandardCompressValue(led_button_BYTEOFFSET,led_button_BITOFFSET,led_button_LEN,(uint8_t *)devStatusPtr,dataPoints->valueled_button);
    gizStandardCompressValue(motov_button_BYTEOFFSET,motov_button_BITOFFSET,motov_button_LEN,(uint8_t *)devStatusPtr,dataPoints->valuemotov_button);
    gizStandardCompressValue(beep_button_BYTEOFFSET,beep_button_BITOFFSET,beep_button_LEN,(uint8_t *)devStatusPtr,dataPoints->valuebeep_button);
    ...
    devStatusPtr->valuesmoke = exchangeBytes(gizY2X(smoke_RATIO,  smoke_ADDITION, dataPoints->valuesmoke)); 

    devStatusPtr->valueethanol = exchangeWord(gizY2XFloat(ethanol_RATIO,  ethanol_ADDITION, dataPoints->valueethanol)); 
