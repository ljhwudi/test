...
int8_t gizwitsEventProcess(eventInfo_t *info, uint8_t *gizdata, uint32_t len)
{
  ...
  for(i=0; i<info->num; i++)
  {
    ...
    {
      case EVENT_W_LED:
        currentDataPoint.valueW_LED = dataPointPtr->valueW_LED;
        GIZWITS_LOG("Evt: EVENT_W_LED %d \n", currentDataPoint.valueW_LED);
        if(0x01 == currentDataPoint.valueW_LED)
        {
          //user handle
        ...
        break;
      case EVENT_R_LED:
        currentDataPoint.valueR_LED = dataPointPtr->valueR_LED;
        GIZWITS_LOG("Evt: EVENT_R_LED %d \n", currentDataPoint.valueR_LED);
        if(0x01 == currentDataPoint.valueR_LED)
        {
          //user handle
